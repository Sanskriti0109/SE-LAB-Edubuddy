...
export default function TeacherDashboard() {
  ...
  const handleAddTask = async (e) => {
    ...
    try {
      ...
      // Make a POST request to update the tasks for the teacher
      const response = await axios.post(
        `${process.env.REACT_APP_BACKEND_URL}/api/teacher/update-task`,
        {
      ...
      // Update the teacher state with the response data
      if (response) {
  ...
  return (
    <>
      <div className="flex h-[90vh]">
        ...
        <div>
          <div className="p-8">
            <p className="text-white text-xl">{quote?.quote}</p>
            {/* <p className="text-white text-sm float-right">{quote?.author}</p> */}
          </div>
